...
const bool _otaa = true;
#else
const bool _otaa = false;
#endif
...
static uint8_t AppEui[] =
    LORAWAN_APPLICATION_EUI;                       /**< Application identifier */
...
bool singleChannelGateway =
    false; /**< Flag if connection is to a single channel gateway */
uint8_t singleChannelSelected =
    0;                               /**< Channel to be used to communicate with a single channel gateway */
int8_t singleChannelDatarate = DR_3; /**< Datarate to be used to communicate
                                        with a single channel gateway */
...
static lmh_compliance_test_t
    m_compliance_test; /**< LoRaWAN compliance tests data */

static bool m_adr_enable_init;
static TimerEvent_t ComplianceTestTxNextPacketTimer;

void lmh_setDevEui(uint8_t userDevEui[]) { memcpy(DevEui, userDevEui, 8); }

void lmh_setAppEui(uint8_t *userAppEui) { memcpy(AppEui, userAppEui, 8); }

void lmh_setAppKey(uint8_t *userAppKey) { memcpy(AppKey, userAppKey, 16); }

void lmh_setNwkSKey(uint8_t *userNwkSKey) { memcpy(NwkSKey, userNwkSKey, 16); }

void lmh_setAppSKey(uint8_t *userAppSKey) { memcpy(AppSKey, userAppSKey, 16); }

void lmh_setDevAddr(uint32_t userDevAddr) { DevAddr = userDevAddr; }
...
bool lmh_setSubBandChannels(uint8_t subBand)
{
  ...
  ESP_LOGI("LMH", "Selected subband %d", subBand);
...
static bool compliance_test_tx(void)
{
  ...
  else
  {
    ...
    case 1:
      ...
      m_compliance_test.data_buffer[0] =
          m_compliance_test.downlink_counter >> 8;
      m_compliance_test.data_buffer[1] = m_compliance_test.downlink_counter;
      break;
    }
  }

  if (LoRaMacQueryTxPossible(m_compliance_test.data_buffer_size, &txInfo) !=
      LORAMAC_STATUS_OK)
  {
 ...
 *
 * @param mcpsConfirm Pointer to the confirm structure, containing confirm
 * attributes.
 */
static void McpsConfirm(McpsConfirm_t *mcpsConfirm)
{
  ...
  if ((mcpsConfirm->Status == LORAMAC_EVENT_INFO_STATUS_RX1_TIMEOUT) ||
       ...
       LORAMAC_EVENT_INFO_STATUS_TX_DR_PAYLOAD_SIZE_ERROR))
  {
    ...
    case MCPS_UNCONFIRMED:
    {
      ...
      // Check TxPower
      // Report unconfirmed TX finished
      ESP_LOGI("LMH", "Timeout TX + RX finished");
      if (m_callbacks->lmh_unconf_finished != 0)
      {
        m_callbacks->lmh_unconf_finished();
      }
    ...
    case MCPS_CONFIRMED:
    {
      ...
      // Report confirmed TX finished with result
      ESP_LOGI("LMH", "Timeout Conf TX finished %s",
               mcpsConfirm->AckReceived ? "SUCC" : "FAIL");
      if (m_callbacks->lmh_conf_result != 0)
      {
        m_callbacks->lmh_conf_result(mcpsConfirm->AckReceived);
      }
 ...
 *
 * @param mcpsIndication	Pointer to the indication structure, containing
    ...
    case LORAWAN_CERTIF_PORT:
      ...
      if (m_compliance_test.running == false)
      {
            ...
            (mcpsIndication->Buffer[3] == 0x01))
        {
          ...
          TimerInit(&ComplianceTestTxNextPacketTimer,
      ...
      else
      {
        ...
        case 4: // (vii)
        {
          ...
          for (uint8_t i = 1; i < T_MIN(m_compliance_test.data_buffer_size,
 ...
 *
 * @param mlmeConfirm	Pointer to the confirm structure, containing confirm
 * attributes.
 */
                          ...
                          LoRaMacRegion_t user_region)
{
  ...
  if (_otaa)
  {
    sprintf(strlog2, "AppEui=%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X",
            AppEui[0], AppEui[1], AppEui[2], AppEui[3], AppEui[4], AppEui[5],
            AppEui[6], AppEui[7]);
    sprintf(strlog1, "DevEui=%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X",
            DevEui[0], DevEui[1], DevEui[2], DevEui[3], DevEui[4], DevEui[5],
            DevEui[6], DevEui[7]);
    sprintf(strlog3,
            "AppKey=%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%"
            "02X-%02X-%02X-%02X-%02X",
  ...
  else
  {
    sprintf(strlog1, "DevEui=%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X",
            ...
            DevEui[6], DevEui[7]);
    sprintf(strlog2,
            "NwkSKey=%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%"
            "02X-%02X-%02X-%02X-%02X",
            NwkSKey[0], NwkSKey[1], NwkSKey[2], NwkSKey[3], NwkSKey[4],
            NwkSKey[5], NwkSKey[6], NwkSKey[7], NwkSKey[8], NwkSKey[9],
            NwkSKey[10], NwkSKey[11], NwkSKey[12], NwkSKey[13], NwkSKey[14],
            NwkSKey[15]);
    sprintf(strlog3,
            "AppSKey=%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%02X-%"
            "02X-%02X-%02X-%02X-%02X",
            AppSKey[0], AppSKey[1], AppSKey[2], AppSKey[3], AppSKey[4],
            AppSKey[5], AppSKey[6], AppSKey[7], AppSKey[8], AppSKey[9],
            AppSKey[10], AppSKey[11], AppSKey[12], AppSKey[13], AppSKey[14],
            AppSKey[15]);
    ESP_LOGI("LMH", "ABP \n%s\nDevAdd=%ld\n%s\n%s", strlog1, DevAddr, strlog2,
             strlog3);
  }
  ...
  ESP_LOGI("DD", "2-4");
  error_status = LoRaMacInitialization(&LoRaMacPrimitives, &LoRaMacCallbacks,
                                       region, nodeClass);
  if (error_status != LORAMAC_STATUS_OK)
  {
    ESP_LOGE("LORA", "LMH_ERROR");
    return LMH_ERROR;
  }
  ESP_LOGI("DD", "3");
  mibReq.Type = MIB_ADR;
  ...
  case LORAMAC_REGION_AS923:
    ...
    ESP_LOGI("LMH", "Using AS923-1");
    break;
  case LORAMAC_REGION_AS923_2:
    lmh_setAS923Version(2);
    ESP_LOGI("LMH", "Using AS923-2");
    break;
  case LORAMAC_REGION_AS923_3:
    lmh_setAS923Version(3);
    ESP_LOGI("LMH", "Using AS923-3");
    break;
  case LORAMAC_REGION_AS923_4:
    lmh_setAS923Version(4);
    ESP_LOGI("LMH", "Using AS923-4");
    break;
  default:
    break;
  }
                          ...
                          lmh_confirm is_tx_confirmed)
{
  ...
  if (LoRaMacQueryTxPossible(app_data->buffsize, &txInfo) !=
      LORAMAC_STATUS_OK)
  {
    ESP_LOGI("LMH", "lmh_send -> LoRaMacQueryTxPossible failed");
  ...
  else
  {
    ...
    ESP_LOGI("LMH", "lmh_send -> LoRaMacMcpsRequest failed");
  }
                                   ...
                                   uint32_t time_out)
{
  if (lmh_send(app_data, is_tx_confirmed) == LMH_SUCCESS)
  {
    ...
    while (lmh_mac_is_busy)
    {
      ...
      {
        ESP_LOGI("LMH", "timeout at %ld", (millis() - time_start));
  ...
  ESP_LOGI("LMH", "lmh_send returned LMH_ERROR");
  return LMH_ERROR;
}
...
uint32_t lmh_getDevAddr(void) { return LoRaMacGetOTAADevId(); }
...
uint8_t lmh_getConfRetries(void) { return max_ack_retries; }