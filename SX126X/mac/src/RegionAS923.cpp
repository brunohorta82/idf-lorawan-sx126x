...
void RegionAS923ApplyCFList(ApplyCFListParams_t *applyCFList)
{
	...
	for (uint8_t i = 0, chanIdx = AS923_NUMB_DEFAULT_CHANNELS; chanIdx < AS923_MAX_NB_CHANNELS; i += 3, chanIdx++)
	{
		if (chanIdx < (AS923_NUMB_CHANNELS_CF_LIST + AS923_NUMB_DEFAULT_CHANNELS))
		{
			...
			ESP_LOGI("AS923", "Apply CF list: new channel at Freq = %ld", newChannel.Frequency);
		}
		else
		{
...
bool RegionAS923TxConfig(TxConfigParams_t *txConfig, int8_t *txPower, TimerTime_t *txTimeOnAir)
{
	...
	ESP_LOGI("AS923", "Using TX Frequency %ld", Channels[txConfig->Channel].Frequency);
...
LoRaMacStatus_t RegionAS923ChannelAdd(ChannelAddParams_t *channelAdd)
{
	...
	if (id < AS923_NUMB_DEFAULT_CHANNELS)
	{
		...
		{
			ESP_LOGI("AS923", "DR to low %d", channelAdd->NewChannel->DrRange.Fields.Min);
		...
		{
			ESP_LOGI("AS923", "DR to high %d", channelAdd->NewChannel->DrRange.Fields.Max);
			drInvalid = true;
		}
		// We are not allowed to change the frequency
		// if (channelAdd->NewChannel->Frequency != Channels[id].Frequency)
		// {
		// 	ESP_LOGI("AS923", "New channel change frequency failed %.1f", (channelAdd->NewChannel->Frequency)/100000);
		// 	freqInvalid = true;
		// }
	}
	...
	if (freqInvalid == false)
	{
		...
		{
			ESP_LOGI("AS923", "New channel not accepted %.1f", (float)((channelAdd->NewChannel->Frequency) / 1000000.0));
			freqInvalid = true;
		}
	}
	...
	{
		ESP_LOGI("AS923", "New channel not accepted FREQ_AND_DR_INVALID");
		return LORAMAC_STATUS_FREQ_AND_DR_INVALID;
	}
	if (drInvalid == true)
	{
		ESP_LOGI("AS923", "New channel not accepted DATARATE_INVALID");
		return LORAMAC_STATUS_DATARATE_INVALID;
	}
	if (freqInvalid == true)
	{
		ESP_LOGI("AS923", "New channel not accepted FREQUENCY_INVALID");
		return LORAMAC_STATUS_FREQUENCY_INVALID;
	}

	ESP_LOGI("AS923", "New channel added %.1f", (float)((channelAdd->NewChannel->Frequency) / 1000000.0));
...
bool RegionAS923SetVersion(uint8_t version)
{
	...
	{
	case 1:
		ESP_LOGI("AS923", "Switch to Version 1");
		return true;
		break;
	case 2:
		ESP_LOGI("AS923", "Switch to Version 2");
		...
		break;
	case 3:
		ESP_LOGI("AS923", "Switch to Version 3");
		for (int idx = 0; idx < 2; idx++)
		{
		...
		break;
	case 4:
		ESP_LOGI("AS923", "Switch to Version 4");
		for (int idx = 0; idx < 2; idx++)
		{
		...
		break;
	default:
		ESP_LOGI("AS923", "Wrong version request");
		return false;
	}
	for (int idx = 0; idx < 2; idx++)
	{
		ESP_LOGI("AS923", "CH%d - TX %.1f", idx, Channels[idx].Frequency / 1000000.0);
	}
	ESP_LOGI("AS923", "RX2 %.1f", AS923_RX_WND_2_FREQ / 1000000.0);
	return true;
}
