...
static uint8_t CountNbOfEnabledChannels(bool joined, uint8_t datarate, uint16_t *channelsMask, ChannelParams_t *channels, Band_t *bands, uint8_t *enabledChannels, uint8_t *delayTx)
{
	...
	for (uint8_t i = 0, k = 0; i < EU868_MAX_NB_CHANNELS; i += 16, k++)
	{
		for (uint8_t j = 0; j < 16; j++)
		{
			...
			{
				ESP_LOGI("EU868", "Channel count ch# %d, freq %ld", i + j, channels[i + j].Frequency);
				...
				ESP_LOGI("EU868", "Set channel %d, frequency %ld", nbEnabledChannels - 1, channels[i + j].Frequency);
			}
		}
	}
...
void RegionEU868ApplyCFList(ApplyCFListParams_t *applyCFList)
{
	...
	for (uint8_t i = 0, chanIdx = EU868_NUMB_DEFAULT_CHANNELS; chanIdx < EU868_MAX_NB_CHANNELS; i += 3, chanIdx++)
	{
		if (chanIdx < (EU868_NUMB_CHANNELS_CF_LIST + EU868_NUMB_DEFAULT_CHANNELS))
		{
			...
			ESP_LOGI("EU868", "Apply CF list: new channel at Freq = %ld", newChannel.Frequency);
...
uint8_t RegionEU868NewChannelReq(NewChannelReqParams_t *newChannelReq)
{
	...
	else
	{
		...
		ESP_LOGI("EU868", "Requested DR was %d", newChannelReq->NewChannel->DrRange.Fields.Max);

		if (newChannelReq->NewChannel->DrRange.Fields.Max < 0)
		{
			ESP_LOGI("EU868", "Requested DR was %d, changed to %d", newChannelReq->NewChannel->DrRange.Fields.Max, abs(newChannelReq->NewChannel->DrRange.Fields.Max));
		...
		{
			ESP_LOGI("EU868", "New Channel Request accepted");
			break;
		}
		case LORAMAC_STATUS_FREQUENCY_INVALID:
		{
			ESP_LOGI("EU868", "New Channel Request frequency invalid");
			status &= 0xFE;
			break;
		}
		case LORAMAC_STATUS_DATARATE_INVALID:
		{
			ESP_LOGI("EU868", "New Channel Request DR invalid");
			status &= 0xFD;
			break;
		}
		case LORAMAC_STATUS_FREQ_AND_DR_INVALID:
		{
			ESP_LOGI("EU868", "New Channel Request frequency & DR invalid");
			status &= 0xFC;
			break;
		}
		default:
		{
			ESP_LOGI("EU868", "New Channel Request unknown failure");
			status &= 0xFC;
			break;
		}
		}
...
bool RegionEU868NextChannel(NextChanParams_t *nextChanParams, uint8_t *channel, TimerTime_t *time, TimerTime_t *aggregatedTimeOff)
{
	...
	if (nbEnabledChannels > 0)
	{
		...
		*channel = enabledChannels[randr(0, nbEnabledChannels - 1)];
		ESP_LOGI("EU868", "Using channel %d, frequency %ld", channel[0], Channels[channel[0]].Frequency);
		*time = 0;
		return true;
	}
	else
